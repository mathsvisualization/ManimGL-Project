...
class WaveExtrapolation(InteractiveScene):
    def construct(self):
        # 1. Mathematical curves: (x(t), y(t)) jahan t [0, 2*PI] vary karega
        curves_data = [
            {
                "name": "Circle",
                "color": BLUE,
        ...
        for data in curves_data:
            ...
            def get_wave_points():
                ...
                # Wave points history banate hue
                t_samples = np.linspace(max(0, t_current - TAU), t_current, 150)
                points = []
                for t in t_samples:
                    # Shape se y-offset fetch karna
                    y_val = func(t % TAU)[1] * 0.8 + base_pos[1]
                    # Rightward movement: current dot position ke right side unfold hona
            ...
            # Connecting dashed line from dot to wave start
            ...
            self.add(shape, dot, conn_line, wave, label)

        # 2. Animation loop (2 cycles)
        self.play(
